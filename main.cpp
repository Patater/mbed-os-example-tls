#include "mbed.h"

#include "mbedtls/config.h"

#include "mbedtls/aes.h"
#include "mbedtls/sha1.h"
#include "mbedtls/sha256.h"
#include "mbedtls/des.h"
#include "mbedtls/md5.h"

#include "mbedtls/platform.h"
#include "mbedtls/error.h"

#define REPETITIONS 5

#if !defined(MBEDTLS_AES_ALT)
#warning "AES hardware acceleration is not enabled!"
#endif

#if !defined(MBEDTLS_SHA1_ALT)
#warning "SHA1 hardware acceleration is not enabled!"
#endif

#if !defined(MBEDTLS_SHA256_ALT)
#warning "SHA256 hardware acceleration is not enabled!"
#endif

#if !defined(MBEDTLS_MD5_ALT)
#warning "MD5 hardware acceleration is not enabled!"
#endif

/*****************************************************************************/
/*************************AES-CBC tests***************************************/
/*****************************************************************************/

#define AES_CTX_COUNT 12

const unsigned char keys[AES_CTX_COUNT][16] = {
    { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xf8, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfc, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0xe3, 0x7b, 0x1c, 0x6a, 0xa2, 0x84, 0x6f, 0x6f,
      0xdb, 0x41, 0x3f, 0x23, 0x8b, 0x08, 0x9f, 0x23, },
    { 0x6c, 0x00, 0x2b, 0x68, 0x24, 0x83, 0xe0, 0xca,
      0xbc, 0xc7, 0x31, 0xc2, 0x53, 0xbe, 0x56, 0x74, },
    { 0x14, 0x3a, 0xe8, 0xed, 0x65, 0x55, 0xab, 0xa9,
      0x61, 0x10, 0xab, 0x58, 0x89, 0x3a, 0x8a, 0xe1, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
};

const unsigned char inputs[AES_CTX_COUNT][16] = {
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, },
    { 0x6a, 0x11, 0x8a, 0x87, 0x45, 0x19, 0xe6, 0x4e,
      0x99, 0x63, 0x79, 0x8a, 0x50, 0x3f, 0x1d, 0x35, },
    { 0xcb, 0x9f, 0xce, 0xec, 0x81, 0x28, 0x6c, 0xa3,
      0xe9, 0x89, 0xbd, 0x97, 0x9b, 0x0c, 0xb2, 0x84, },
    { 0xb2, 0x6a, 0xeb, 0x18, 0x74, 0xe4, 0x7c, 0xa8,
      0x35, 0x8f, 0xf2, 0x23, 0x78, 0xf0, 0x91, 0x44, },
    { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xc0, 0x00, 0x00, 0x00, 0x00, },
    { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xe0, 0x00, 0x00, 0x00, 0x00, },
    { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xf0, 0x00, 0x00, 0x00, 0x00, },
};

const unsigned char results[AES_CTX_COUNT][16] = {
    { 0x8b, 0x52, 0x7a, 0x6a, 0xeb, 0xda, 0xec, 0x9e,
      0xae, 0xf8, 0xed, 0xa2, 0xcb, 0x77, 0x83, 0xe5, },
    { 0x43, 0xfd, 0xaf, 0x53, 0xeb, 0xbc, 0x98, 0x80,
      0xc2, 0x28, 0x61, 0x7d, 0x6a, 0x9b, 0x54, 0x8b, },
    { 0x53, 0x78, 0x61, 0x04, 0xb9, 0x74, 0x4b, 0x98,
      0xf0, 0x52, 0xc4, 0x6f, 0x1c, 0x85, 0x0d, 0x0b, },
    { 0x43, 0xc9, 0xf7, 0xe6, 0x2f, 0x5d, 0x28, 0x8b,
      0xb2, 0x7a, 0xa4, 0x0e, 0xf8, 0xfe, 0x1e, 0xa8, },
    { 0x35, 0x80, 0xd1, 0x9c, 0xff, 0x44, 0xf1, 0x01,
      0x4a, 0x7c, 0x96, 0x6a, 0x69, 0x05, 0x9d, 0xe5, },
    { 0x80, 0x6d, 0xa8, 0x64, 0xdd, 0x29, 0xd4, 0x8d,
      0xea, 0xfb, 0xe7, 0x64, 0xf8, 0x20, 0x2a, 0xef, },
    { 0xdc, 0x43, 0xbe, 0x40, 0xbe, 0x0e, 0x53, 0x71,
      0x2f, 0x7e, 0x2b, 0xf5, 0xca, 0x70, 0x72, 0x09, },
    { 0x92, 0xbe, 0xed, 0xab, 0x18, 0x95, 0xa9, 0x4f,
      0xaa, 0x69, 0xb6, 0x32, 0xe5, 0xcc, 0x47, 0xce, },
    { 0x45, 0x92, 0x64, 0xf4, 0x79, 0x8f, 0x6a, 0x78,
      0xba, 0xcb, 0x89, 0xc1, 0x5e, 0xd3, 0xd6, 0x01, },
    { 0x90, 0x68, 0x4a, 0x2a, 0xc5, 0x5f, 0xe1, 0xec,
      0x2b, 0x8e, 0xbd, 0x56, 0x22, 0x52, 0x0b, 0x73, },
    { 0x74, 0x72, 0xf9, 0xa7, 0x98, 0x86, 0x07, 0xca,
      0x79, 0x70, 0x77, 0x95, 0x99, 0x10, 0x35, 0xe6, },
    { 0x56, 0xaf, 0xf0, 0x89, 0x87, 0x8b, 0xf3, 0x35,
      0x2f, 0x8d, 0xf1, 0x72, 0xa3, 0xae, 0x47, 0xd8, },
};

int aes_cbc()
{
    int ret;
    mbedtls_aes_context *ctx;
    size_t i, j;
    unsigned char output[16];
    unsigned char iv[16];

    if( ( ctx = (mbedtls_aes_context *)mbedtls_calloc( AES_CTX_COUNT,
                                    sizeof( mbedtls_aes_context ) ) ) == NULL )
    {
        mbedtls_printf( "  !  mbedtls_calloc() returned NULL\n" );
        return( -1 );
    }

    /* Initialise all context structures */
    for( i = 0; i < AES_CTX_COUNT; i++ )
        mbedtls_aes_init( &ctx[i] );

    /* Set keys */
    for( i = 0; i < AES_CTX_COUNT; i++ )
        if( ( ret = mbedtls_aes_setkey_enc( &ctx[i], keys[i], 128 ) ) != 0 )
        {
            mbedtls_printf( "  !  mbedtls_aes_setkey_enc() returned -0x%04X\n",
                            ret );
            goto exit;
        }

    /* Encrypt values... a few times */
    for( j = 0; j < REPETITIONS; j++ )
        for( i = 0; i < AES_CTX_COUNT; i++ )
        {
            memset( iv, 0x00, sizeof( iv ) );

            if( ( ret = mbedtls_aes_crypt_cbc( &ctx[i], MBEDTLS_AES_ENCRYPT,
                                               16, iv,
                                               inputs[i], output ) ) != 0 )
            {
                mbedtls_printf( "%d:  !  mbedtls_aes_crypt_cbc() returned "
                                "-0x%04X\n", __LINE__, ret );
                goto exit;
            }

            if( memcmp( output, results[i], 16 ) != 0 )
            {
                mbedtls_printf( "%d:  !  memcmp() result is not 0 at "
                                " repetition %u:%u\n",
                                __LINE__, j, i );
                ret = -2;
                goto exit;
            }
        }

    ret = 0;

exit:
    for( i = 0 ; i < AES_CTX_COUNT; i++ )
        mbedtls_aes_free( &ctx[i] );

    mbedtls_free( ctx );

    return( ret );
}

/*****************************************************************************/
/*******************************SHA1******************************************/
/*****************************************************************************/

#define SHA1_CTX_COUNT 10

unsigned char sha1_inputs[SHA1_CTX_COUNT][658] = {
    { },
    { 0xa8 },
    { 0x30, 0x00 },
    { 0x42, 0x74, 0x9e},
    { 0x9f, 0xc3, 0xfe, 0x08 },
    { 0xb5, 0xc1, 0xc6, 0xf1, 0xaf },
    { , },
    { , },
    { , },
    { , },
};

size_t sha1_ilens[SHA1_CTX_COUNT] = {
    0, 1, 2, 3, 4, 5, 163, 262, 361, 658,
};

unsigned char sha1_results[SHA1_CTX_COUNT][20] = {
    { 0xda, 0x39, 0xa3, 0xee, 0x5e, 0x6b, 0x4b, 0x0d,
      0x32, 0x55, 0xbf, 0xef, 0x95, 0x60, 0x18, 0x90,
      0xaf, 0xd8, 0x07, 0x09, },
    { 0x99, 0xf2, 0xaa, 0x95, 0xe3, 0x6f, 0x95, 0xc2,
      0xac, 0xb0, 0xea, 0xf2, 0x39, 0x98, 0xf0, 0x30,
      0x63, 0x8f, 0x3f, 0x15, },
    { 0xf9, 0x44, 0xdc, 0xd6, 0x35, 0xf9, 0x80, 0x1f,
      0x7a, 0xc9, 0x0a, 0x40, 0x7f, 0xbc, 0x47, 0x99,
      0x64, 0xde, 0xc0, 0x24, },
    { 0xa4, 0x44, 0x31, 0x9e, 0x9b, 0x6c, 0xc1, 0xe8,
      0x46, 0x4c, 0x51, 0x1e, 0xc0, 0x96, 0x9c, 0x37,
      0xd6, 0xbb, 0x26, 0x19, },
    { 0x16, 0xa0, 0xff, 0x84, 0xfc, 0xc1, 0x56, 0xfd,
      0x5d, 0x3c, 0xa3, 0xa7, 0x44, 0xf2, 0x0a, 0x23,
      0x2d, 0x17, 0x22, 0x53, },
    { 0xfe, 0xc9, 0xde, 0xeb, 0xfc, 0xde, 0xda, 0xf6,
      0x6d, 0xda, 0x52, 0x5e, 0x1b, 0xe4, 0x35, 0x97,
      0xa7, 0x3a, 0x1f, 0x93, },
    { 0x97, 0x01, 0x11, 0xc4, 0xe7, 0x7b, 0xcc, 0x88,
      0xcc, 0x20, 0x45, 0x9c, 0x02, 0xb6, 0x9b, 0x4a,
      0xa8, 0xf5, 0x82, 0x17, },
    { 0x04, 0x23, 0xdc, 0x76, 0xa8, 0x79, 0x11, 0x07,
      0xd1, 0x4e, 0x13, 0xf5, 0x26, 0x5b, 0x34, 0x3f,
      0x24, 0xcc, 0x0f, 0x19, },
    { 0x66, 0x92, 0xa7, 0x1d, 0x73, 0xe0, 0x0f, 0x27,
      0xdf, 0x97, 0x6b, 0xc5, 0x6d, 0xf4, 0x97, 0x06,
      0x50, 0xd9, 0x0e, 0x45, },
    { 0x11, 0x86, 0x3b, 0x48, 0x38, 0x09, 0xef, 0x88,
      0x41, 0x3c, 0xa9, 0xb0, 0x08, 0x4a, 0xc4, 0xa5,
      0x39, 0x06, 0x40, 0xaf, },
};

#define SPLIT 5

int sha1()
{
    int ret, next;
    size_t i, j;
    mbedtls_sha1_context *ctx;
    unsigned char output[20];
    size_t proc_len[SHA1_CTX_COUNT];
    size_t next_len;

    if( ( ctx = (mbedtls_sha1_context *)mbedtls_calloc( SHA1_CTX_COUNT,
                                    sizeof( mbedtls_sha1_context ) ) ) == NULL )
    {
        mbedtls_printf( "  !  mbedtls_calloc() return NULL\n" );
        return( -1 );
    }

    /* Initialise all context structures */
    for( i = 0; i < SHA1_CTX_COUNT; i++ )
        mbedtls_sha1_init( &ctx[i] );

    for( j = 1; j < REPETITIONS; j++ )
    {
        /* Start the contexts */
        for( i = 0; i < SHA1_CTX_COUNT; i++ )
            mbedtls_sha1_starts( &ctx[i] );

        memset( proc_len, 0, sizeof( proc_len ) );

        do
        {
            next = 0;

            for( i = 0; i < SHA1_CTX_COUNT; i++ )
            {
                /* Nothing left to do */
                if( proc_len[i] == sha1_ilens[i] )
                    continue;

                next = 1;

                next_len = sha1_ilens[i] / SPLIT + 1;
                next_len = ( next_len + proc_len[i] <= sha1_ilens[i] ) ? next_len :
                            sha1_ilens[i] - proc_len[i];

                mbedtls_sha1_update( &ctx[i], sha1_inputs[i] + proc_len[i],
                                    next_len );
                proc_len[i] += next_len;
            }
        }
        while( next == 1 );

        /* Verify the outputs */
        for( i = 0; i < SHA1_CTX_COUNT; i++ )
        {
            mbedtls_sha1_finish( &ctx[i], output );

            if( memcmp( output, sha1_results[i], sizeof( output ) ) != 0 )
            {
                mbedtls_printf( "%d:  !  memcmp() result is not 0 at "
                                " repetition %u:%u\n",
                                __LINE__, j, i );
                ret = -2;
                goto exit;
            }
        }
    }

    ret = 0;

exit:
    for( i = 0; i < SHA1_CTX_COUNT; i++ )
        mbedtls_sha1_free( &ctx[i] );

    mbedtls_free( ctx );

    return( ret );
}

/*****************************************************************************/
/*******************************SHA256****************************************/
/*****************************************************************************/

#define SHA256_CTX_COUNT 7

unsigned char sha256_inputs[SHA256_CTX_COUNT][955] = {
    { },
    { 0xbd },
    { 0x5f, 0xd4 },
    { 0xb0, 0xbd, 0x69 },
    { 0xc9, 0x8c, 0x8e, 0x55, },
    { 0x81, 0xa7, 0x23, 0xd9, 0x66, },
    { , },
};

size_t sha256_ilens[SHA256_CTX_COUNT] = {
    0, 1, 2, 3, 4, 5, 955
};

unsigned char sha256_results[SHA256_CTX_COUNT][32] = {
    { 0xe3, 0xb0, 0xc4, 0x42, 0x98, 0xfc, 0x1c, 0x14,
      0x9a, 0xfb, 0xf4, 0xc8, 0x99, 0x6f, 0xb9, 0x24,
      0x27, 0xae, 0x41, 0xe4, 0x64, 0x9b, 0x93, 0x4c,
      0xa4, 0x95, 0x99, 0x1b, 0x78, 0x52, 0xb8, 0x55, },
    { 0x68, 0x32, 0x57, 0x20, 0xaa, 0xbd, 0x7c, 0x82,
      0xf3, 0x0f, 0x55, 0x4b, 0x31, 0x3d, 0x05, 0x70,
      0xc9, 0x5a, 0xcc, 0xbb, 0x7d, 0xc4, 0xb5, 0xaa,
      0xe1, 0x12, 0x04, 0xc0, 0x8f, 0xfe, 0x73, 0x2b, },
    { 0x7c, 0x4f, 0xbf, 0x48, 0x44, 0x98, 0xd2, 0x1b,
      0x48, 0x7b, 0x9d, 0x61, 0xde, 0x89, 0x14, 0xb2,
      0xea, 0xda, 0xf2, 0x69, 0x87, 0x12, 0x93, 0x6d,
      0x47, 0xc3, 0xad, 0xa2, 0x55, 0x8f, 0x67, 0x88, },
    { 0x40, 0x96, 0x80, 0x42, 0x21, 0x09, 0x3d, 0xdc,
      0xcf, 0xbf, 0x46, 0x83, 0x14, 0x90, 0xea, 0x63,
      0xe9, 0xe9, 0x94, 0x14, 0x85, 0x8f, 0x8d, 0x75,
      0xff, 0x7f, 0x64, 0x2c, 0x7c, 0xa6, 0x18, 0x03, },
    { 0x7a, 0xbc, 0x22, 0xc0, 0xae, 0x5a, 0xf2, 0x6c,
      0xe9, 0x3d, 0xbb, 0x94, 0x43, 0x3a, 0x0e, 0x0b,
      0x2e, 0x11, 0x9d, 0x01, 0x4f, 0x8e, 0x7f, 0x65,
      0xbd, 0x56, 0xc6, 0x1c, 0xcc, 0xcd, 0x95, 0x04, },
    { 0x75, 0x16, 0xfb, 0x8b, 0xb1, 0x13, 0x50, 0xdf,
      0x2b, 0xf3, 0x86, 0xbc, 0x3c, 0x33, 0xbd, 0x0f,
      0x52, 0xcb, 0x4c, 0x67, 0xc6, 0xe4, 0x74, 0x5e,
      0x04, 0x88, 0xe6, 0x2c, 0x2a, 0xea, 0x26, 0x05, },
    { 0x41, 0x09, 0xcd, 0xbe, 0xc3, 0x24, 0x0a, 0xd7,
      0x4c, 0xc6, 0xc3, 0x7f, 0x39, 0x30, 0x0f, 0x70,
      0xfe, 0xde, 0x16, 0xe2, 0x1e, 0xfc, 0x77, 0xf7,
      0x86, 0x59, 0x98, 0x71, 0x4a, 0xad, 0x0b, 0x5e, },
};

#if 0
struct md_thread_ctx {
    uint32_t wait_ms;
    mbedtls_sha256_context *md_ctx;
};

void md_thread(void *ctx)
{
    md_thread_ctx *c = reinterpret_cast<md_thread_ctx *>(ctx);

    Thread::wait(c->wait_ms);

    switch (ctx->op)
    {
    case MD_THREAD_SHA256_INIT:
    {
        break;
    }
    case MD_THREAD_SHA256_STARTS:
    {
        break;
    }
    case MD_THREAD_SHA256_UPDATE:
    {
        break;
    }
    case MD_THREAD_SHA256_FINISH:
    {
        break;
    }
    default:
    {
        mbedtls_printf("Unknown thread op\n");
        break;
    }
    }
}
#endif

int sha256()
{
    int ret, next;
    size_t i, j;
    mbedtls_sha256_context *ctx;
    unsigned char output[32];
    size_t proc_len[SHA256_CTX_COUNT];
    size_t next_len;

    if( ( ctx = (mbedtls_sha256_context *)mbedtls_calloc( SHA256_CTX_COUNT,
                                    sizeof( mbedtls_sha256_context ) ) ) == NULL )
    {
        mbedtls_printf( "  !  mbedtls_calloc() return NULL\n" );
        return( -1 );
    }

    /* Initialise all context structures */
    for( i = 0; i < SHA256_CTX_COUNT; i++ )
        mbedtls_sha256_init( &ctx[i] );

    for( j = 1; j < REPETITIONS; j++ )
    {
        /* Start the contexts */
        for( i = 0; i < SHA256_CTX_COUNT; i++ )
            mbedtls_sha256_starts( &ctx[i], 0 );

        memset( proc_len, 0, sizeof( proc_len ) );

        do
        {
            next = 0;

            for( i = 0; i < SHA256_CTX_COUNT; i++ )
            {
                /* Nothing left to do */
                if( proc_len[i] == sha256_ilens[i] )
                    continue;

                next = 1;

                next_len = sha256_ilens[i] / SPLIT + 1;
                next_len = ( next_len + proc_len[i] <= sha256_ilens[i] ) ? next_len :
                            sha256_ilens[i] - proc_len[i];

                mbedtls_sha256_update( &ctx[i], sha256_inputs[i] + proc_len[i],
                                    next_len );
                proc_len[i] += next_len;
            }
        }
        while( next == 1 );

        /* Verify the outputs */
        for( i = 0; i < SHA256_CTX_COUNT; i++ )
        {
            mbedtls_sha256_finish( &ctx[i], output );

            if( memcmp( output, sha256_results[i], sizeof( output ) ) != 0 )
            {
                mbedtls_printf( "%d:  !  memcmp() result is not 0 at "
                                " repetition %u:%u\n",
                                __LINE__, j, i );
                ret = -2;
                goto exit;
            }
        }
    }

    ret = 0;

exit:
    for( i = 0; i < SHA256_CTX_COUNT; i++ )
        mbedtls_sha256_free( &ctx[i] );

    mbedtls_free( ctx );

    return( ret );
}

#define SHA256_SP_CTX_COUNT 3

const unsigned char sha256_sp_input[SHA256_SP_CTX_COUNT][64] = {
    {
    0x1E, 0xB9, 0xE7, 0x6C, 0xA6, 0x51, 0x23, 0xAA,
    0xE6, 0x5F, 0xE7, 0xD2, 0x46, 0x34, 0xD4, 0xB3,
    0xC6, 0xED, 0x8E, 0xD4, 0x50, 0xDD, 0xD3, 0xBC,
    0xCD, 0x21, 0x99, 0x36, 0x6F, 0xD6, 0x35, 0x15,
    0x4C, 0x6C, 0x31, 0xAE, 0x4D, 0x9B, 0x29, 0xCF,
    0x31, 0x7E, 0x15, 0x76, 0x5F, 0x84, 0x3C, 0xEE,
    0x6B, 0x68, 0x10, 0xC5, 0x09, 0xB4, 0x18, 0x9C,
    0x9E, 0xFB, 0xED, 0xCE, 0xEF, 0x20, 0xD0, 0xBA
    },
    {
    0x18, 0x78, 0x72, 0x08, 0x55, 0xC1, 0x14, 0xE6,
    0xF0, 0x5D, 0x3A, 0x26, 0x29, 0xCD, 0xEB, 0x9A,
    0x70, 0x06, 0xC1, 0x20, 0xCA, 0x54, 0xCD, 0x4F,
    0xF6, 0x81, 0xE3, 0x1F, 0xF4, 0xD0, 0x92, 0xBC,
    0x6E, 0xA4, 0x39, 0xAD, 0xA7, 0x8E, 0x90, 0x0D,
    0xD8, 0x10, 0xFF, 0x3A, 0x06, 0x98, 0x4A, 0x41,
    0xC8, 0x72, 0xF5, 0x89, 0xFC, 0xCC, 0x02, 0x90,
    0x6B, 0x33, 0x07, 0x49, 0x20, 0x05, 0xEC, 0xC6
    },
    {
    0x52, 0xB5, 0xF5, 0x53, 0x12, 0xB7, 0x05, 0x0E,
    0x49, 0x92, 0xDE, 0xC8, 0x45, 0xCA, 0x97, 0xD1,
    0xE4, 0x94, 0x5D, 0x0A, 0x6A, 0x54, 0x72, 0xFF,
    0x88, 0xC9, 0xB7, 0xFA, 0x3F, 0x35, 0x51, 0x70,
    0xCB, 0xA3, 0x50, 0x2E, 0x09, 0x3B, 0x6B, 0x23,
    0x16, 0xEF, 0xFC, 0xF8, 0xB1, 0xF5, 0x87, 0xF4,
    0x24, 0x04, 0x07, 0x41, 0x92, 0x81, 0x67, 0x78,
    0x18, 0xDF, 0x47, 0xE5, 0xCD, 0xAC, 0x83, 0x7D
    }
};

const unsigned char sha256_sp_output[SHA256_SP_CTX_COUNT][32] = {
    {
    0xce, 0x26, 0xe4, 0xcf, 0xcf, 0x05, 0xaa, 0xbd,
    0xc5, 0x2b, 0x56, 0x3c, 0xe0, 0x44, 0xb3, 0x3a,
    0x3f, 0x92, 0xde, 0xae, 0x2c, 0xe1, 0xf2, 0xf5,
    0x37, 0xaf, 0x4c, 0xdd, 0xde, 0xb9, 0x8e, 0x7c
    },
    {
    0x65, 0xdb, 0xf4, 0x41, 0x96, 0x27, 0x16, 0x77,
    0x28, 0x28, 0x1a, 0x54, 0xb4, 0xda, 0x8b, 0xb7,
    0x1e, 0xf1, 0xcd, 0x85, 0x69, 0xd6, 0x77, 0x2e,
    0xe3, 0xf9, 0x45, 0xf6, 0x07, 0x88, 0x3a, 0xbf
    },
    {
    0xba, 0x3e, 0xd5, 0x9d, 0x90, 0x7e, 0x1c, 0xc9,
    0x0b, 0xc0, 0x2d, 0x23, 0x96, 0x46, 0x1f, 0x22,
    0x43, 0xf8, 0x80, 0x96, 0x0a, 0xca, 0x24, 0x53,
    0x1f, 0x4a, 0x94, 0x5d, 0x3d, 0x11, 0x72, 0xe2
    }
};

const unsigned char big_buffer[652] = { 0 };
mbedtls_sha256_context ctx_0x20005120;
mbedtls_sha256_context ctx_0x2001424c;
mbedtls_sha256_context ctx_0x20005208;
mbedtls_sha256_context ctx_0x20005280;
mbedtls_sha256_context ctx_0x20005268;
mbedtls_sha256_context ctx_0x200052f8;
mbedtls_sha256_context ctx_0x2001dac4;
mbedtls_sha256_context ctx_0x20005a48;
mbedtls_sha256_context ctx_0x2001e3d8;
mbedtls_sha256_context ctx_0x2001f1e8;
mbedtls_sha256_context ctx_0x20005bc8;
mbedtls_sha256_context ctx_0x2001f358;
mbedtls_sha256_context ctx_0x20005bb0;
mbedtls_sha256_context ctx_0x2001eac0;
mbedtls_sha256_context ctx_0x20005a78;

int sha256_special2()
{
    unsigned char XXXoutput[32];
    const unsigned char *XXXinput = big_buffer;

    mbedtls_sha256_init( &ctx_0x20005120 );
    mbedtls_sha256_starts_ret( &ctx_0x20005120, 0 );
    mbedtls_sha256_update_ret( &ctx_0x20005120, XXXinput, 128 );
    mbedtls_sha256_finish_ret( &ctx_0x20005120, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005120 );
    mbedtls_sha256_starts_ret( &ctx_0x2001424c, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001424c, XXXinput, 2 );
    mbedtls_sha256_update_ret( &ctx_0x2001424c, XXXinput, 32 );
    mbedtls_sha256_init( &ctx_0x20005120 );
    mbedtls_sha256_starts_ret( &ctx_0x20005120, 0 );
    mbedtls_sha256_update_ret( &ctx_0x20005120, XXXinput, 128 );
    mbedtls_sha256_finish_ret( &ctx_0x20005120, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005120 );
    mbedtls_sha256_update_ret( &ctx_0x2001424c, XXXinput, 2 );
    mbedtls_sha256_update_ret( &ctx_0x2001424c, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001424c, XXXoutput );
    mbedtls_sha256_free( &ctx_0x2001424c );
    mbedtls_sha256_init( &ctx_0x2001424c );
    mbedtls_sha256_starts_ret( &ctx_0x2001424c, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001424c, XXXinput, 32 );
    mbedtls_sha256_init( &ctx_0x20005208 );
    mbedtls_sha256_starts_ret( &ctx_0x20005208, 0 );
    mbedtls_sha256_update_ret( &ctx_0x20005208, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x20005208, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005208 );
    mbedtls_sha256_init( &ctx_0x20005280 );
    mbedtls_sha256_starts_ret( &ctx_0x20005280, 0 );
    mbedtls_sha256_update_ret( &ctx_0x20005280, XXXinput, 27 );
    mbedtls_sha256_finish_ret( &ctx_0x20005280, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005280 );
    mbedtls_sha256_init( &ctx_0x20005268 );
    mbedtls_sha256_starts_ret( &ctx_0x20005268, 0 );
    mbedtls_sha256_update_ret( &ctx_0x20005268, XXXinput, 27 );
    mbedtls_sha256_finish_ret( &ctx_0x20005268, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005268 );
    mbedtls_sha256_init( &ctx_0x20005280 );
    mbedtls_sha256_starts_ret( &ctx_0x20005280, 0 );
    mbedtls_sha256_update_ret( &ctx_0x20005280, XXXinput, 33 );
    mbedtls_sha256_finish_ret( &ctx_0x20005280, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005280 );
    mbedtls_sha256_init( &ctx_0x20005268 );
    mbedtls_sha256_starts_ret( &ctx_0x20005268, 0 );
    mbedtls_sha256_update_ret( &ctx_0x20005268, XXXinput, 33 );
    mbedtls_sha256_finish_ret( &ctx_0x20005268, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005268 );
    mbedtls_sha256_init( &ctx_0x200052f8 );
    mbedtls_sha256_starts_ret( &ctx_0x200052f8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x200052f8, XXXinput, 25 );
    mbedtls_sha256_finish_ret( &ctx_0x200052f8, XXXoutput );
    mbedtls_sha256_free( &ctx_0x200052f8 );
    mbedtls_sha256_init( &ctx_0x2001dac4 );
    mbedtls_sha256_starts_ret( &ctx_0x2001dac4, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 102 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 47 );
    mbedtls_sha256_starts_ret( &ctx_0x2001dac4, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 134 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 99 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 652 );
    mbedtls_sha256_init( &ctx_0x20005a48 );
    mbedtls_sha256_starts_ret( &ctx_0x20005a48, 0 );
    mbedtls_sha256_update_ret( &ctx_0x20005a48, XXXinput, 543 );
    mbedtls_sha256_finish_ret( &ctx_0x20005a48, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005a48 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 157 );
    mbedtls_sha256_init( &ctx_0x2001e3d8 );
    mbedtls_sha256_starts_ret( &ctx_0x2001e3d8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001e3d8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001e3d8, XXXinput, 69 );
    mbedtls_sha256_finish_ret( &ctx_0x2001e3d8, XXXoutput );
    mbedtls_sha256_free( &ctx_0x2001e3d8 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 31 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 12 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 644 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 78 );
    mbedtls_sha256_init( &ctx_0x20005a48 );
    mbedtls_sha256_clone( &ctx_0x20005a48, &ctx_0x2001dac4 );
    mbedtls_sha256_finish_ret( &ctx_0x20005a48, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005a48 );
    mbedtls_sha256_init( &ctx_0x2001f1e8 );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 54 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 86 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 86 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_free( &ctx_0x2001f1e8 );
    mbedtls_sha256_init( &ctx_0x2001f1e8 );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 77 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 109 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 109 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 109 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 109 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 109 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 109 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 109 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 109 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f1e8, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f1e8, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f1e8, XXXoutput );
    mbedtls_sha256_free( &ctx_0x2001f1e8 );
    mbedtls_sha256_init( &ctx_0x20005bc8 );
    mbedtls_sha256_clone( &ctx_0x20005bc8, &ctx_0x2001dac4 );
    mbedtls_sha256_finish_ret( &ctx_0x20005bc8, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005bc8 );
    mbedtls_sha256_init( &ctx_0x2001f358 );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 1 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 1 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 1 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 1 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 1 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001f358, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001f358, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001f358, XXXoutput );
    mbedtls_sha256_free( &ctx_0x2001f358 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 88 );
    mbedtls_sha256_init( &ctx_0x20005bb0 );
    mbedtls_sha256_clone( &ctx_0x20005bb0, &ctx_0x2001dac4 );
    mbedtls_sha256_finish_ret( &ctx_0x20005bb0, XXXoutput );
    mbedtls_sha256_init( &ctx_0x2001eac0 );
    mbedtls_sha256_starts_ret( &ctx_0x2001eac0, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 47 );
    mbedtls_sha256_finish_ret( &ctx_0x2001eac0, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001eac0, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001eac0, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001eac0, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 79 );
    mbedtls_sha256_finish_ret( &ctx_0x2001eac0, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001eac0, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001eac0, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001eac0, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001eac0, XXXoutput );
    mbedtls_sha256_starts_ret( &ctx_0x2001eac0, 0 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 64 );
    mbedtls_sha256_update_ret( &ctx_0x2001eac0, XXXinput, 32 );
    mbedtls_sha256_finish_ret( &ctx_0x2001eac0, XXXoutput );
    mbedtls_sha256_free( &ctx_0x2001eac0 );
    mbedtls_sha256_free( &ctx_0x20005bb0 );
    mbedtls_sha256_update_ret( &ctx_0x2001dac4, XXXinput, 24 );

    printf("long pause...\n");

    mbedtls_sha256_init( &ctx_0x20005a78 );
    mbedtls_sha256_starts_ret( &ctx_0x20005a78, 0 );
    mbedtls_sha256_update_ret( &ctx_0x20005a78, XXXinput, 26 );
    mbedtls_sha256_finish_ret( &ctx_0x20005a78, XXXoutput );
    mbedtls_sha256_free( &ctx_0x20005a78 );
    mbedtls_sha256_free( &ctx_0x2001dac4 );

    return( 0 );
}

int sha256_special3()
{
    int ret = 0;
    mbedtls_sha256_context *ctx;
    unsigned char output[32];

    enum { SHA256_SP3_CTX_COUNT = 1 };

    if( ( ctx = (mbedtls_sha256_context *)mbedtls_calloc( SHA256_SP3_CTX_COUNT,
                                    sizeof( mbedtls_sha256_context ) ) ) == NULL )
    {
        mbedtls_printf( "  !  mbedtls_calloc() return NULL\n" );
        return( -1 );
    }

    /* Start an operation and feed in a few bytes. */
    mbedtls_sha256_init( &ctx[0] );
    mbedtls_sha256_starts( &ctx[0], 0 );
    mbedtls_sha256_update( &ctx[0], sha256_sp_input[0], 64 );

    /* Finish with the orig context and verify the result. */
    mbedtls_sha256_finish( &ctx[0], output );
    if( memcmp( output, sha256_sp_output[0], sizeof( output ) ) != 0 )
    {
        printf( "%s:%u:  !  memcmp() result is not 0",
                __FILE__, __LINE__ );
        ret = -2;
        goto exit;
    }
    mbedtls_sha256_free( &ctx[0] );

    /* Re-init the ctx */
    mbedtls_sha256_init( &ctx[0] );
    mbedtls_sha256_starts( &ctx[0], 0 );
    mbedtls_sha256_update( &ctx[0], sha256_sp_input[2], 64 );

    /* Finish with the orig context and verify the result. */
    mbedtls_sha256_finish( &ctx[0], output );
    if( memcmp( output, sha256_sp_output[2], sizeof( output ) ) != 0 )
    {
        printf( "%s:%u:  !  memcmp() result is not 0",
                __FILE__, __LINE__ );
        ret = -2;
        goto exit;
    }
    mbedtls_sha256_free( &ctx[0] );

exit:
    mbedtls_free( ctx );
    return( ret );
}

int sha256_special4()
{
    /* Getting a start after an update */
    int ret = 0;
    mbedtls_sha256_context *ctx;
    unsigned char output[32];

    enum { SHA256_SP3_CTX_COUNT = 1 };

    if( ( ctx = (mbedtls_sha256_context *)mbedtls_calloc( SHA256_SP3_CTX_COUNT,
                                    sizeof( mbedtls_sha256_context ) ) ) == NULL )
    {
        mbedtls_printf( "  !  mbedtls_calloc() return NULL\n" );
        return( -1 );
    }

    /* Start an operation and feed in a few bytes. */
    mbedtls_sha256_init( &ctx[0] );
    mbedtls_sha256_starts( &ctx[0], 0 );
    mbedtls_sha256_update( &ctx[0], sha256_sp_input[0], 64 );

    /* Do a repeated start and feed in different bytes */
    mbedtls_sha256_starts( &ctx[0], 0 );
    mbedtls_sha256_update( &ctx[0], sha256_sp_input[2], 64 );

    /* Finish with the orig context and verify the result, which should only be
     * calculated based on the second issue of bytes. */
    mbedtls_sha256_finish( &ctx[0], output );
    if( memcmp( output, sha256_sp_output[2], sizeof( output ) ) != 0 )
    {
        printf( "%s:%u:  !  memcmp() result is not 0",
                __FILE__, __LINE__ );
        ret = -2;
        goto exit;
    }
    mbedtls_sha256_free( &ctx[0] );

exit:
    mbedtls_free( ctx );
    return( ret );
}

int sha256_special()
{
    int ret;
    mbedtls_sha256_context *ctx;
    unsigned char output[32];

    printf("\n-----------------SPECIAL TEST---------------------\n");

    if( ( ctx = (mbedtls_sha256_context *)mbedtls_calloc( SHA256_SP_CTX_COUNT,
                                    sizeof( mbedtls_sha256_context ) ) ) == NULL )
    {
        mbedtls_printf( "  !  mbedtls_calloc() return NULL\n" );
        return( -1 );
    }

    /* Start an operation and feed in a few bytes. */
    mbedtls_sha256_init( &ctx[0] );
    mbedtls_sha256_starts( &ctx[0], 0 );
    mbedtls_sha256_update( &ctx[0], sha256_sp_input[0], 64 );

    /* Clone the context to a new one. */
    mbedtls_sha256_clone( &ctx[1], &ctx[0] );

    /* Finish with the new context and verify the result. */
    mbedtls_sha256_finish( &ctx[1], output );
    if( memcmp( output, sha256_sp_output[0], sizeof( output ) ) != 0 )
    {
        mbedtls_printf( "%s:%u:  !  memcmp() result is not 0",
                        __FILE__, __LINE__ );
        ret = -2;
        goto exit;
    }

    /* Start and finish and entire unrelated operation, and verify the result.
     * */
    mbedtls_sha256_init( &ctx[2] );
    mbedtls_sha256_starts( &ctx[2], 0 );
    mbedtls_sha256_update( &ctx[2], sha256_sp_input[2], 64 );
    mbedtls_sha256_finish( &ctx[2], output );
    if( memcmp( output, sha256_sp_output[2], sizeof( output ) ) != 0 )
    {
        mbedtls_printf( "%s:%u:  !  memcmp() result is not 0",
                        __FILE__, __LINE__ );
        ret = -2;
        goto exit;
    }

    /* Free the unrelated context */
    mbedtls_sha256_free( &ctx[2] );

    /* Free the initial cloned context, since we are done with it. */
    mbedtls_sha256_free( &ctx[1] );

    /* Add some more bytes to the original context and then finish it,
     * verifying the result. */
    mbedtls_sha256_update( &ctx[0], sha256_sp_input[1], 64 );
    mbedtls_sha256_finish( &ctx[0], output );
    if( memcmp( output, sha256_sp_output[1], sizeof( output ) ) != 0 )
    {
        mbedtls_printf( "%s:%u:  !  memcmp() result is not 0",
                        __FILE__, __LINE__ );
        ret = -2;
        goto exit;
    }

#if 0
    /*
     * clone the context to new1
     * finish with the new1 context
     * init new2 context
     * do operations with new2 context
     * free new2 context
     * free new1 context
     * do operations with original context
     *
     */

    for( j = 1; j < REPETITIONS; j++ )
    {
        /* Start the contexts */
        for( i = 0; i < SHA256_SP_CTX_COUNT; i++ )
            mbedtls_sha256_starts( &ctx[i], 0 );

        memset( proc_len, 0, sizeof( proc_len ) );

        do
        {
            next = 0;

            for( i = 0; i < SHA256_SP_CTX_COUNT; i++ )
            {
                /* Nothing left to do */
                if( proc_len[i] == sha256_ilens[i] )
                    continue;

                next = 1;

                next_len = sha256_ilens[i] / SPLIT + 1;
                next_len = ( next_len + proc_len[i] <= sha256_ilens[i] ) ? next_len :
                            sha256_ilens[i] - proc_len[i];

                mbedtls_sha256_update( &ctx[i], sha256_inputs[i] + proc_len[i],
                                    next_len );
                proc_len[i] += next_len;
            }
        }
        while( next == 1 );

        /* Verify the outputs */
        for( i = 0; i < SHA256_SP_CTX_COUNT; i++ )
        {
            mbedtls_sha256_finish( &ctx[i], output );

            if( memcmp( output, sha256_results[i], sizeof( output ) ) != 0 )
            {
                mbedtls_printf( "%d:  !  memcmp() result is not 0 at "
                                " repetition %u:%u\n",
                                __LINE__, j, i );
                ret = -2;
                goto exit;
            }
        }
    }

#endif
    ret = 0;

exit:
    mbedtls_free( ctx );

    printf("\n----------------END SPECIAL TEST------------------\n");

    return( ret );
}

/*****************************************************************************/
/*******************************MD5*******************************************/
/*****************************************************************************/

#define MD5_CTX_COUNT 7

unsigned char md5_inputs[MD5_CTX_COUNT][80] = {
    { },
    { 0x61 },
    { 0x61, 0x62, 0x63 },
    { 0x6d, 0x65, 0x73, 0x73, 0x61, 0x67, 0x65, 0x20,
      0x64, 0x69, 0x67, 0x65, 0x73, 0x74 },
    { 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68,
      0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f, 0x70,
      0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78,
      0x79, 0x7a },
    { 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48,
      0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f, 0x50,
      0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58,
      0x59, 0x5a, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66,
      0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e,
      0x6f, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76,
      0x77, 0x78, 0x79, 0x7a, 0x30, 0x31, 0x32, 0x33,
      0x34, 0x35, 0x36, 0x37, 0x38, 0x39 },
    { 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38,
      0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36,
      0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34,
      0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32,
      0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30,
      0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38,
      0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36,
      0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34,
      0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32,
      0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30 },
};

size_t md5_ilens[MD5_CTX_COUNT] = {
    0, 1, 3, 14, 26, 62, 80,
};

unsigned char md5_results[MD5_CTX_COUNT][16] = {
    { 0xd4, 0x1d, 0x8c, 0xd9, 0x8f, 0x00, 0xb2, 0x04,
      0xe9, 0x80, 0x09, 0x98, 0xec, 0xf8, 0x42, 0x7e, },
    { 0x0c, 0xc1, 0x75, 0xb9, 0xc0, 0xf1, 0xb6, 0xa8,
      0x31, 0xc3, 0x99, 0xe2, 0x69, 0x77, 0x26, 0x61, },
    { 0x90, 0x01, 0x50, 0x98, 0x3c, 0xd2, 0x4f, 0xb0,
      0xd6, 0x96, 0x3f, 0x7d, 0x28, 0xe1, 0x7f, 0x72, },
    { 0xf9, 0x6b, 0x69, 0x7d, 0x7c, 0xb7, 0x93, 0x8d,
      0x52, 0x5a, 0x2f, 0x31, 0xaa, 0xf1, 0x61, 0xd0, },
    { 0xc3, 0xfc, 0xd3, 0xd7, 0x61, 0x92, 0xe4, 0x00,
      0x7d, 0xfb, 0x49, 0x6c, 0xca, 0x67, 0xe1, 0x3b, },
    { 0xd1, 0x74, 0xab, 0x98, 0xd2, 0x77, 0xd9, 0xf5,
      0xa5, 0x61, 0x1c, 0x2c, 0x9f, 0x41, 0x9d, 0x9f, },
    { 0x57, 0xed, 0xf4, 0xa2, 0x2b, 0xe3, 0xc9, 0x55,
      0xac, 0x49, 0xda, 0x2e, 0x21, 0x07, 0xb6, 0x7a, },
};

int md5()
{
    int ret, next;
    size_t i, j;
    mbedtls_md5_context *ctx;
    unsigned char output[16];
    size_t proc_len[MD5_CTX_COUNT];
    size_t next_len;

    if( ( ctx = (mbedtls_md5_context *)mbedtls_calloc( MD5_CTX_COUNT,
                                    sizeof( mbedtls_md5_context ) ) ) == NULL )
    {
        mbedtls_printf( "  !  mbedtls_calloc() return NULL\n" );
        return( -1 );
    }

    /* Initialise all context structures */
    for( i = 0; i < MD5_CTX_COUNT; i++ )
        mbedtls_md5_init( &ctx[i] );

    for( j = 1; j < REPETITIONS; j++ )
    {
        /* Start the contexts */
        for( i = 0; i < MD5_CTX_COUNT; i++ )
            mbedtls_md5_starts( &ctx[i] );

        memset( proc_len, 0, sizeof( proc_len ) );

        do
        {
            next = 0;

            for( i = 0; i < MD5_CTX_COUNT; i++ )
            {
                /* Nothing left to do */
                if( proc_len[i] == md5_ilens[i] )
                    continue;

                next = 1;

                next_len = md5_ilens[i] / SPLIT + 1;
                next_len = ( next_len + proc_len[i] <= md5_ilens[i] ) ? next_len :
                            md5_ilens[i] - proc_len[i];

                mbedtls_md5_update( &ctx[i], md5_inputs[i] + proc_len[i],
                                    next_len );
                proc_len[i] += next_len;
            }
        }
        while( next == 1 );

        /* Verify the outputs */
        for( i = 0; i < MD5_CTX_COUNT; i++ )
        {
            mbedtls_md5_finish( &ctx[i], output );

            if( memcmp( output, md5_results[i], sizeof( output ) ) != 0 )
            {
                mbedtls_printf( "%d:  !  memcmp() result is not 0 at "
                                " repetition %u:%u\n",
                                __LINE__, j, i );
                ret = -2;
                goto exit;
            }
        }
    }

    ret = 0;

exit:
    for( i = 0; i < MD5_CTX_COUNT; i++ )
        mbedtls_md5_free( &ctx[i] );

    mbedtls_free( ctx );

    return( ret );
}

int main()
{
    int ret;
    int exit_code = MBEDTLS_EXIT_FAILURE;

    wait(1);
    printf("Welcome to Mbed TLS Crypto Stress Test\n");
    wait(1);

    if( ( ret = aes_cbc() ) != 0 )
        mbedtls_printf( "failed\n  ! AES-CBC returned %d\n", ret );
    if( ( ret = sha1() ) != 0 )
        mbedtls_printf( "failed\n  ! SHA-1 returned %d\n", ret );
    //if( ( ret = sha256() ) != 0 )
    //    mbedtls_printf( "failed\n  ! SHA-256 returned %d\n", ret );
    //if( ( ret = sha256_special() ) != 0 )
    //    mbedtls_printf( "failed\n  ! SHA-256 special returned %d\n", ret );
    //if( ( ret = sha256_special2() ) != 0 )
    //    mbedtls_printf( "failed\n  ! SHA-256 special 2 returned %d\n", ret );
    //if( ( ret = sha256_special3() ) != 0 )
    //    mbedtls_printf( "failed\n  ! SHA-256 special 3 returned %d\n", ret );
    if( ( ret = sha256_special4() ) != 0 )
        mbedtls_printf( "failed\n  ! SHA-256 special 4 returned %d\n", ret );
    if( ( ret = md5() ) != 0 )
        mbedtls_printf( "failed\n  ! MD5 returned %d\n", ret );

    if( ret != 0 )
        mbedtls_printf( "FAIL\n" );
    else
    {
        mbedtls_printf( "DONE\n" );
        exit_code = MBEDTLS_EXIT_SUCCESS;
    }

    for(;;);

    return( exit_code );
}
